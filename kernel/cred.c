...
#ifdef CONFIG_RKP_KDP
	if(rkp_cred_enable){
		volatile cred_param_t cred_param;
		...
		__flush_dcache_area((void *)&cred_param, sizeof(cred_param));
		uh_call(UH_APP_RKP, 0x46, (u64)&cred_param, 0, 0, 0);
		if((new_ro->bp_task != p) 
			|| new_ro->security != tsec 
			|| new_ro->use_cnt != use_cnt_ptr) {
			panic("Copy Creds: RKP Call failed cred_ro = %p:%llx rkp_ro_start = %llx:%llx task=#%p:%p#, sec=#%p:%p#, usecnt=#%p:%p#",new_ro,(u64)__pa((u64)new_ro),(u64)__rkp_ro_start,(u64)__pa((u64)__rkp_ro_start),new_ro->bp_task,p,new_ro->security,tsec,new_ro->use_cnt,use_cnt_ptr);
		}
...
int commit_creds(struct cred *new)
{
	...
	if(rkp_cred_enable) {
		...
		if((new_ro->bp_task != current)||
			...
			(new_ro->security != tsec) ||
			(new_ro->use_cnt != use_cnt_ptr)) {
			panic("Commit Creds: RKP Call failed cred_ro = %p:%llx rkp_ro_start = %llx:%llx  task=#%p:%p#, sec=#%p:%p#, usecnt=#%p:%p#, pgd=#%p:%p#",new_ro,(u64)__pa((u64)new_ro),(u64)__rkp_ro_start,(u64)__pa((u64)__rkp_ro_start),new_ro->bp_task,current,new_ro->security,tsec,new_ro->use_cnt,use_cnt_ptr,new_ro->bp_pgd,(void *)pgd);
		}
...
#endif  /* CONFIG_RKP_KDP */
{
	...
	if(rkp_cred_enable) {
		...
		if((new_ro->bp_task != current)||
			...
			(new_ro->security != tsec) ||
			(new_ro->use_cnt != use_cnt_ptr)) {
			panic("Override Creds: RKP Call failed cred_ro = %p:%llx rkp_ro_start = %llx:%llx  task=#%p:%p#, sec=#%p:%p#, usecnt=#%p:%p#, pgd=#%p:%p#",new_ro,(u64)__pa((u64)new_ro),(u64)__rkp_ro_start,(u64)__pa((u64)__rkp_ro_start),new_ro->bp_task,current,new_ro->security,tsec,new_ro->use_cnt,use_cnt_ptr,new_ro->bp_pgd,(void *)pgd);
		}
		rocred_uc_set(new_ro,2);
