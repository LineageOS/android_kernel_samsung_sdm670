...
#define SEC_ADAPTATAION_DSP_SRC_PORT		5	/* AFE */
#define SEC_ADAPTATAION_DSP_DEST_PORT		0	/* AFE */
...
static int q6audio_tfadsp_read(int dev, int buf_size, char *buf)
{
	...
	if (this_afe.apr == NULL) {
		this_afe.apr = apr_register("ADSP", "AFE",
				...
				SEC_ADAPTATAION_DSP_SRC_PORT, &this_afe);
	}
	...
	tfa_dsp_read_msg.hdr.src_port = SEC_ADAPTATAION_DSP_SRC_PORT;
	tfa_dsp_read_msg.hdr.dest_port = SEC_ADAPTATAION_DSP_DEST_PORT;
		...
		int dev, int buf_size, char *buf, int msg_type, int num_msgs)
{
	...
	afe_cal.hdr.src_port = SEC_ADAPTATAION_DSP_SRC_PORT;
	afe_cal.hdr.dest_port = SEC_ADAPTATAION_DSP_DEST_PORT;
	...
	int dev, int buf_size, char *buf, int msg_type, int num_msgs)
{
	...
	*/
	...
	tfa_dsp_send_msg->hdr.src_port = SEC_ADAPTATAION_DSP_SRC_PORT;
	tfa_dsp_send_msg->hdr.dest_port = SEC_ADAPTATAION_DSP_DEST_PORT;
...
int q6asm_set_dolby_atmos(struct audio_client *ac, long *param)
{
	...
	cmd.param.data_payload_size =
			sizeof(cmd) - sizeof(cmd.hdr) - sizeof(cmd.param);
	cmd.data.module_id = ASM_MODULE_ID_PP_DOLBY_DAP;
	cmd.data.param_id = ASM_PARAM_ID_PP_DOLBY_DAP_PARAMS;
	cmd.data.param_size = cmd.param.data_payload_size - sizeof(cmd.data);
	cmd.data.reserved = 0;

	/* upscaler paramerters */
	cmd.dolby_enable = param[0];
	cmd.dolby_device = param[1];
	cmd.dolby_profile = param[2];

	pr_info("%s: %d %d %d\n", __func__,
		cmd.dolby_enable, cmd.dolby_device, cmd.dolby_profile);
	...
	uint32_t sb_enable)
{
	...
	pr_info("%s: port_id 0x%x, copp_idx %d, sb_enable %d\n",
			__func__, port_id, copp_idx, sb_enable);

	params_value = kzalloc(param_length, GFP_KERNEL);
	if (!params_value)	{
		pr_err("%s: failed to alloc memory for params_value err=%d\n",
			__func__, ret);
		return -ENOMEM;
	}
	...
	/* add command */
	*update_params_value++ = ADM_MODULE_ID_PP_SB;
	*update_params_value++ = ADM_PARAM_ID_PP_SB_PARAM;
	*update_params_value++ = sizeof(uint32_t);
	*update_params_value++ = sb_enable;

	ret = adm_send_params_v5(port_id, copp_idx,
				params_value, param_length);
	if (ret) {
			...
			uint32_t sb_rotation)
{
	...
	pr_info("%s: port_id 0x%x, copp_idx %d, sb_rotation %d\n",
			__func__, port_id, copp_idx, sb_rotation);
	...
	/* add command */
	*update_params_value++ = ADM_MODULE_ID_PP_SB;
	*update_params_value++ = ADM_PARAM_ID_PP_SB_ROTATION_PARAM;
	*update_params_value++ = sizeof(uint32_t);
	*update_params_value++ = sb_rotation;
			...
			struct snd_ctl_elem_value *ucontrol)
{
	...
	uint32_t sb_enable = ucontrol->value.integer.value[0];


	port_id = afe_port.amp_rx_id;
	ret = q6audio_get_copp_idx_from_port_id(port_id, 1, (int)sb_enable, &copp_idx);
	if (ret) {
	...
	ret = adm_set_sound_booster(port_id, copp_idx, sb_enable);
	if (ret) {
		pr_err("%s: Error setting Sound boost, err=%d\n",
			  __func__, ret);
			...
			struct snd_ctl_elem_value *ucontrol)
{
	int ret = 0;
	int port_id, copp_idx;
	uint32_t sb_rotation = ucontrol->value.integer.value[0];

	port_id = afe_port.amp_rx_id;
	ret = q6audio_get_copp_idx_from_port_id(port_id, 2, (int)sb_rotation, &copp_idx);
	if (ret) {
		pr_err("%s: Could not get copp idx for port_id=%d\n",
			__func__, port_id);
	...
	ret = adm_set_sb_rotation(port_id, copp_idx, sb_rotation);
	if (ret) {
