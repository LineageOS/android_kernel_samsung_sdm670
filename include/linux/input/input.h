...
#define set_core_num(core_num){ \
	if (core_num != current_core_num) { \
		pr_booster("[Input Booster2] ******      set_core_num : %d ( %s )\n", core_num, __FUNCTION__); \
		if (core_ctl_set_boost(core_num != 0) < 0) { \
			pr_booster("[Input Booster2] ******            !!! fail to set core num!!!\n"); \
		} else { \
...
#define SET_BOOSTER  { \
	...
	set_hmp(value); \
	MAX_T_INPUT_BOOSTER(value, core_num); \
	if (value == INPUT_BOOSTER_NULL) { \
		value = 0; \
	} \
	set_core_num(value); \
...
#define PROPERTY_BOOSTER(_device_param_, _dt_param_, _time_)  { \
    _device_param_.cpu_freq = _dt_param_.cpu_freq; \
	_device_param_.core_num = _dt_param_.core_num; \
...
#define SYSFS_COPY_TO_FROM(_A_, _B_) { \
    _A_ cpu_freq = (*cpu_freq == (unsigned int)(-1)) ? _B_ cpu_freq : *cpu_freq; \
	_A_ core_num = (*core_num == (unsigned int)(-1)) ? _B_ core_num : *core_num; \
...
#define DTSI_TO { \
    err |= of_property_read_u32_index(cnp, "input_booster,cpu_freqs", i, &dt_infor->param_tables[i].cpu_freq); \
	err |= of_property_read_u32_index(cnp, "input_booster,core_num", i, &temp); dt_infor->param_tables[i].core_num = (u8)temp; \
...
#define SYSFS_DEVICE(_ATTR_, _ARGU_, _COUNT_) \
    ...
    static ssize_t input_booster_sysfs_device_store_##_ATTR_(struct device *dev, struct device_attribute *attr, const char *buf, size_t count) { \
        ...
        if (len != _COUNT_) { \
            pr_booster("### Keep this format : [level cpu_freq kfc_freq mif_freq int_freq hmp_boost] (Ex: 1 1600000 0 1500000 667000 333000 1###\n"); \
...
#elif defined(CONFIG_ARCH_QCOM) //______________________________________________________________________________
SYSFS_CLASS(debug_level, (buf, "%u\n", debug_level), 1)
SYSFS_CLASS(head, (buf, "%u %u %u %u %u %u\n", head_time, cpu_freq, bimc_freq, core_num, hmp_boost, lpm_bias), 6)
SYSFS_CLASS(tail, (buf, "%u %u %u %u %u %u\n", tail_time, cpu_freq, bimc_freq, core_num, hmp_boost, lpm_bias), 6)
SYSFS_CLASS(level, (buf, "%d\n", level), 1)
SYSFS_DEVICE(level, (buf, "%d\n", level), 1)
SYSFS_DEVICE(freq, (buf, "%d %u %u %u %u %u\n", level, cpu_freq, bimc_freq, core_num, hmp_boost, lpm_bias), 6)
