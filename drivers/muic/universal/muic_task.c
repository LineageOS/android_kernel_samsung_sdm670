...
int muic_get_otg_property(void)
{
	...
	if (psy_otg) {
		val.intval = 0;
		ret = power_supply_get_property(psy_otg, POWER_SUPPLY_PROP_ONLINE, &val);
	} else {
...
static int muic_irq_handler_afc(muic_data_t *pmuic, int irq)
{
	...
	if (intr1 & MUIC_INT_ATTACH_MASK)
	{
		...
		pr_info("3. %s: intr1[0x%x]\n", __func__, intr1);
		intr1 |= intr_tmp;
		pr_info("4. %s: intr1[0x%x]\n", __func__, intr1);
	}
	...
	if (intr1 & MUIC_INT_DETACH_MASK) {
		...
		ret = muic_i2c_read_byte(i2c, REG_DEVT1);
		if (ret != 0x40)
	...
	if (pmuic->pdata->afc_limit_voltage) {
		...
		switch (intr3) {
			case INT3_AFC_TA_ATTACHED_MASK:
				pr_info("%s: This interrupt occured after siop function\n", __func__);
	...
	if (pmuic->afc_water_disable) {
		pr_err("%s: Ignore AFC INTS, AFC is disabled by water\n", __func__);
		return INT_REQ_DONE;
	}
		...
		if (pmuic->is_ccic_attach) {
 			if (pmuic->ccic_rp == Rp_56K) {
				...
				if (muic_get_otg_property() == 1) {
					pr_info("%s: Skip during OTG operation\n", __func__);
				} else {
				...
				const struct i2c_device_id *id)
{
	...
	wake_lock_init(&pmuic->wlock, WAKE_LOCK_SUSPEND, "sm5705q-irq");
#endif
#if defined(CONFIG_MUIC_UNIVERSAL_SM5705_AFC)
...
#if !defined(CONFIG_SEC_FACTORY) && defined(CONFIG_MUIC_SUPPORT_RUSTPROOF)
