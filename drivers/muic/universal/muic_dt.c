...
int of_muic_dt(struct i2c_client *i2c, struct muic_platform_data *pdata)
{
	...
	ret = gpio_request(pdata->irq_gpio, "gpio_muic_irq");
...
#ifdef CONFIG_MUIC_SM5705_SWITCH_CONTROL_GPIO
	sm5705_switch_gpio = of_get_named_gpio(np_muic, "muic,sm5705_switch_gpio", 0);
	ret = gpio_is_valid(sm5705_switch_gpio);
	if (!ret) {
		pr_err("sm5705_switch_gpio is not valid!!!\n");
		pmuic->sm5705_switch_gpio = 0;
		return ret;
	} else {
		pr_info("%s: sm5705_switch_gpio: %d\n", __func__, sm5705_switch_gpio);
	}

	pmuic->sm5705_switch_gpio = sm5705_switch_gpio;
#endif
#ifdef CONFIG_MUIC_POGO
	pmuic->mux_sel = of_get_named_gpio(np_muic, "muic,mux_sel", 0);
	ret = gpio_is_valid(pmuic->mux_sel);
	if (!ret) {
		pr_err("mux_sel is not valid!!!\n");
		return ret;
	} else {
