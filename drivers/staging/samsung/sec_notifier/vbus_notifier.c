...
static int vbus_notifier_init_done = 0;
...
static int create_notify_clain(void)
{
        int  ret = 0;
...
{
	int ret = 0;
        if (!vbus_notifier_init_done) {
                create_notify_clain();
                vbus_notifier_init_done = 1;
	        pr_info("%s: init vbus_notifier(%d)\n", __func__, vbus_notifier_init_done);
        }
...
arch_initcall(vbus_notifier_init);
