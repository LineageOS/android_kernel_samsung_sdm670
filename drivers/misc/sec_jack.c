...
#define MAX_MOISTURE_DEBOUNCE	10
#define MAX_MOISTURE_COUNT	10
...
struct sec_jack_info {
	...
	struct delayed_work moisture_work;
	struct workqueue_struct *detect_wq;
	struct workqueue_struct *buttons_wq;
	struct workqueue_struct *moisture_wq;
...
static int sec_jack_gpio_init(struct sec_jack_platform_data *pdata)
{
	...
	if (pdata->gnd_det_en > 0) {
		ret = gpio_request(pdata->gnd_det_en, "gnd_det_en");
		if (ret < 0) {
			pr_err("%s: failed to request gnd_det_en\n", __func__);
			goto err_request_gnd_det_en;
		}
		gpio_direction_output(pdata->gnd_det_en, 1);
	}

	if (pdata->ldet_en_gpio > 0) {
		pr_info("%s: ldet_en_gpio is enable\n", __func__);
		ret = gpio_request(pdata->ldet_en_gpio, "ldet_en_gpio");
		if (ret < 0) {
			pr_err("%s: failed to request ldet_en_gpio\n", __func__);
			goto err_request_ldet_en;
		}
		gpio_direction_output(pdata->ldet_en_gpio, 1);
	}
...
err_request_ldet_en:
	if (pdata->gnd_det_en)
		gpio_free(pdata->gnd_det_en);
err_request_gnd_det_en:
	if (pdata->ear_micbias_en_gpio)
		gpio_free(pdata->ear_micbias_en_gpio);
...
static ssize_t earjack_moisture_adc_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	struct sec_jack_info *hi = dev_get_drvdata(dev);
	struct sec_jack_platform_data *pdata = hi->pdata;
	...
	if (pdata->jack_controls->get_moisture_adc) {
		adc_val = pdata->jack_controls->get_moisture_adc();
		dev_info(&hi->client->dev, "%s: %d\n", __func__, adc_val);
	}
...
static DEVICE_ATTR(moisture_adc, 0444, earjack_moisture_adc_show,
	NULL);
...
int sec_jack_get_curr_state(struct sec_jack_platform_data *pdata)
{
	enum sec_jack_state state = SEC_JACK_NONE;
	int jack_in_cnt = 0;
	int jack_out_cnt = 0;
	int water_cnt = 0;
	int adc_val, i;

	for (i = 0; i < MAX_MOISTURE_DEBOUNCE; i++) {
		adc_val = pdata->jack_controls->get_moisture_adc();
		pr_info("%s: moisture_adc: %d\n",
			__func__, adc_val);

		if (adc_val >= pdata->mdet_range[1])
			jack_out_cnt++;
		else if (adc_val < pdata->mdet_range[0])
			jack_in_cnt++;
		else
			water_cnt++;
	...
	if (jack_out_cnt >= MAX_MOISTURE_COUNT)
		state = SEC_JACK_NONE;
	else if (jack_in_cnt >= MAX_MOISTURE_COUNT)
		state = SEC_JACK_EARJACK;
	else if (water_cnt >= MAX_MOISTURE_COUNT)
		state = SEC_JACK_MOISTURE;
	else
		state = SEC_JACK_ABNORMAL;

	pr_info("%s: state(%d)\n", __func__, state);
	return state;
}

void sec_jack_moisture_work(struct work_struct *work)
{
	struct sec_jack_info *hi =
		container_of(work, struct sec_jack_info, moisture_work.work);
	struct sec_jack_platform_data *pdata = hi->pdata;
	enum sec_jack_state state = SEC_JACK_NONE;

	state = sec_jack_get_curr_state(pdata);

	dev_info(&hi->client->dev, "%s: earjack type(%d)\n",
	...
	case SEC_JACK_NONE:
		...
		if (hi->is_moisture) {
			dev_info(&hi->client->dev,
				"%s: earjack is removed. finish polling, enable irq",
				__func__);
			if (pdata->ldet_en_gpio > 0)
				gpio_set_value(pdata->ldet_en_gpio, 1);
			enable_irq(hi->det_irq);
			hi->is_moisture = false;
		}
		break;
	case SEC_JACK_EARJACK:
		if (hi->cur_jack_type ==  SEC_JACK_NO_DEVICE) {
			dev_info(&hi->client->dev,
				"%s: earjack is inserted in moisture case.",
				__func__);
			wake_lock_timeout(&hi->det_wake_lock, WAKE_LOCK_TIME);
			determine_jack_type(hi);
		}
		queue_delayed_work(hi->moisture_wq,
			&hi->moisture_work, msecs_to_jiffies(1000));
		break;
	case SEC_JACK_MOISTURE:
		if (hi->cur_jack_type > 0)
			handle_jack_not_inserted(hi);
		...
		queue_delayed_work(hi->moisture_wq,
			&hi->moisture_work, msecs_to_jiffies(1000));
		break;
	case SEC_JACK_ABNORMAL:
		dev_info(&hi->client->dev, "%s: could not determine state",
			__func__);
		queue_delayed_work(hi->moisture_wq,
			&hi->moisture_work, msecs_to_jiffies(1000));
		break;
...
int sec_jack_check_moisture(struct sec_jack_info *hi)
{
	struct sec_jack_platform_data *pdata = hi->pdata;
	enum sec_jack_state state = SEC_JACK_NONE;
	...
	case SEC_JACK_MOISTURE:
		if (!hi->is_moisture) {
			dev_info(&hi->client->dev, "%s: detect water, disable irq.",
				__func__);
			if (pdata->ldet_en_gpio > 0)
				gpio_set_value(pdata->ldet_en_gpio, 0);
			hi->is_moisture = true;
		}
		queue_delayed_work(hi->moisture_wq,
			&hi->moisture_work, msecs_to_jiffies(1000));
...
void sec_jack_detect_work(struct work_struct *work)
{
	...
	if (pdata->use_moisture_det) {
		if (sec_jack_check_moisture(hi) == SEC_JACK_MOISTURE) {
			dev_info(&hi->client->dev,
				"%s: ignore the earjack detection in moisture case.",
				__func__);
			handle_jack_not_inserted(hi);
			return;
		}
	...
	*sec_jack_populate_dt_pdata(struct device *dev)
{
	...
	pdata->ldet_en_gpio = of_get_named_gpio(dev->of_node,
				"ldet-en-gpio", 0);
	if (pdata->ldet_en_gpio < 0)
		dev_dbg(dev, "ldet_en_gpio not found\n");

	pdata->ear_micbias_en_gpio = of_get_named_gpio(dev->of_node,
		"ear-micbias-en-gpio", 0);
	if (pdata->ear_micbias_en_gpio < 0)
		dev_dbg(dev, "ear-micbias-en-gpio not found\n");

	pdata->gnd_det_en =
		of_get_named_gpio(dev->of_node, "gnd-det-en", 0);
	if (pdata->gnd_det_en < 0)
		dev_dbg(dev, "gnd-det-en not found\n");
	...
	if (of_property_read_bool(dev->of_node, "use-moisture-det")) {
		dev_info(dev, "support moisture detection\n");
		pdata->use_moisture_det = true;
	}

	if (!of_property_read_u32_array(dev->of_node, "mdet-range",
			pdata->mdet_range, 2))
		dev_info(dev, "mdet-ranges: %d, %d\n",
				pdata->mdet_range[0],
				pdata->mdet_range[1]);
...
static int sec_jack_probe(struct platform_device *pdev)
{
	...
	if (pdata->use_moisture_det)
		INIT_DELAYED_WORK(&hi->moisture_work, sec_jack_moisture_work);
	...
	if (pdata->use_moisture_det) {
		hi->moisture_wq = create_singlethread_workqueue("moisture_wq");
		if (hi->moisture_wq == NULL) {
			ret = -ENOMEM;
			dev_err(&pdev->dev, "Failed to create moisture_wq\n");
			goto err_create_moisture_wq_failed;
		}
	}
	...
	ret = device_create_file(earjack, &dev_attr_moisture_adc);
	if (ret) {
		dev_err(&pdev->dev, "Failed to create device file in %s\n",
			dev_attr_moisture_adc.attr.name);
		goto err_dev_attr_moisture_adc;
	}
...
err_input_register_handler:
	if (pdata->use_moisture_det)
		destroy_workqueue(hi->moisture_wq);
err_create_moisture_wq_failed:
...
static int sec_jack_remove(struct platform_device *pdev)
{
	...
	if (hi->pdata->gnd_det_en)
		gpio_free(hi->pdata->gnd_det_en);
