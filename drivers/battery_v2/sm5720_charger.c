...
{
	return sm5720_update_reg(charger->i2c, SM5720_CHG_REG_WDTCNTL, ((val & 0x3) << 1), (0x3 << 1));
...
static int sm5720_CHG_set_ENWATCHDOG(struct sm5720_charger_data *charger, bool enable)
{
	...
	if (enable) {
		do {
			ret = sm5720_CHG_set_WATCHDOG_TMR(charger, WATCHDOG_TIMER_120s);
			cnt++;
			if (ret != 0)
				pr_info("%s: i2c fail retry! %d\n", __func__, cnt);
		} while (ret != 0 && cnt < 3);
		if (!ret)
		...
		do {
			ret = sm5720_update_reg(charger->i2c,
				SM5720_CHG_REG_WDTCNTL, ((enable & 0x1) << 0), (0x1 << 0));
			cnt++;
			if (ret != 0)
				pr_info("%s: i2c fail retry! %d\n", __func__, cnt);
		} while (ret != 0 && cnt < 3);
...
static int psy_chg_set_cable_online(struct sm5720_charger_data *charger, int cable_type)
{
	...
	if (charger->cable_type == SEC_BATTERY_CABLE_NONE) {
		...
		} else if (is_wireless_type(prev_cable_type)) {
			/* OTG Case after detached the wireless charger: set the default op event and current */
			sm5720_charger_oper_push_event(SM5720_CHARGER_OP_EVENT_VBUSIN, 0);
			sm5720_charger_oper_push_event(SM5720_CHARGER_OP_EVENT_WPCIN, 0);
			sm5720_charger_oper_push_event(SM5720_CHARGER_OP_EVENT_PWR_SHAR, 0);
			sm5720_set_wireless_input_current(charger, 400);
...
static irqreturn_t chg_wpcin_pok_isr(int irq, void *data)
{
	...
	psy_do_property("mfc-charger", set,
		POWER_SUPPLY_EXT_PROP_WIRELESS_DET_IRQ, value);

	return IRQ_HANDLED;
}

static irqreturn_t chg_wpcin_uvlo_isr(int irq, void *data)
{
	struct sm5720_charger_data *charger = data;
	union power_supply_propval value;
...
static inline int _init_sm5720_charger_drv_info(struct sm5720_charger_data *charger)
{
	...
	charger->irq_wpcin_uvlo = pdata->irq_base + SM5720_CHG_IRQ_INT1_WPCINUVLO;
...
static int sm5720_charger_probe(struct platform_device *pdev)
{
	...
	ret = request_threaded_irq(charger->irq_wpcin_uvlo, NULL,
			chg_wpcin_uvlo_isr, 0, "wpc-uvlo-int", charger);
	if (ret) {
		pr_err("fail to request wpcin IRQ: %d: %d\n", charger->irq_wpcin_uvlo, ret);
		goto err_irq;
	}
