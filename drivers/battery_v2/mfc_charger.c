...
void mfc_fod_set_dream_5v(struct mfc_charger_data *charger)
{
	int i = 0;
	u8 fod[12] = {0, };
	...
	if (charger->pdata->fod_dream_5v_data) {
		for(i=0; i< MFC_NUM_FOD_REG; i++) {
			mfc_reg_write(charger->client, MFC_WPC_FOD_0A_REG+i, charger->pdata->fod_dream_5v_data[i]);
		}
		msleep(2);
		for(i=0; i< MFC_NUM_FOD_REG; i++) {
			mfc_reg_read(charger->client, MFC_WPC_FOD_0A_REG+i, &fod[i]);
		}
		pr_info("%s: DREAM 5V FOD(%d %d %d %d %d %d %d %d %d %d %d %d)\n", __func__,
			fod[0], fod[1], fod[2], fod[3], fod[4], fod[5], fod[6], fod[7], fod[8], fod[9], fod[10], fod[11]);
	}
}
		...
		const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_MAX ... POWER_SUPPLY_EXT_PROP_MAX:
		...
		case POWER_SUPPLY_EXT_PROP_FILTER_CFG:
			...
			break;
		case POWER_SUPPLY_EXT_PROP_WIRELESS_DET_IRQ:
			if (charger->pdata->irq_wpc_det < 0) {
				queue_delayed_work(charger->wqueue, &charger->wpc_det_work, msecs_to_jiffies(100));
			}
			break;
		default:
			return -ENODATA;
	...
	if (charger->pdata->cable_type == MFC_PAD_A4WP) {
		...
		if (cmd_data == WPC_TX_COM_AFC_SET) {
			...
			default:
				value.intval = charger->pdata->cable_type;
				if ((charger->pad_vout != PAD_VOUT_10V) &&
					((val_data == TX_ID_DREAM_DOWN) || (val_data == TX_ID_DREAM_STAND))) {
					mfc_fod_set_dream_5v(charger);
				}
		...
		mfc_charger_platform_data_t *pdata)
{
	...
	if (!np) {
		...
		p = of_get_property(np, "battery,fod_dream_5v_data", &len);
		if (p) {
			len = len / sizeof(u32);
			pdata->fod_dream_5v_data = kzalloc(sizeof(*pdata->fod_dream_5v_data) * len, GFP_KERNEL);
			ret = of_property_read_u32_array(np, "battery,fod_dream_5v_data",
				 pdata->fod_dream_5v_data, len);

			for(i = 0; i <len; i++)
				pr_info("%s fod Dream 5V data = 0x%x ",__func__,pdata->fod_dream_5v_data[i]);
		} else {
			pr_err("%s there is not fod_dream_5v_data\n", __func__);
		}
		...
		if (ret < 0) {
			...
			gpio_direction_input(pdata->wpc_int);
...
/*#if !defined(CONFIG_SEC_FACTORY)
	...
	*/
	...
	if (charger->pdata->irq_wpc_det > 0) {
		...
		gpio_direction_input(charger->pdata->wpc_det);
	}
