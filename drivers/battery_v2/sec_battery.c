...
#if defined(CONFIG_SEC_FACTORY) || defined(CONFIG_DUMMY_BATTERY)
int factory_mode;
...
{
	factory_mode = strncmp(val, "2", 1) ? (strncmp(val, "1", 1) ? 0 : 1):2;
...
static void sec_bat_get_charging_current_by_siop(struct sec_battery_info *battery,
	...
	if (battery->siop_level == 3) {
		...
		if (is_wireless_type(battery->cable_type)) {
			...
			if (battery->siop_level == 0) { /* 3 step(0) */
				max_charging_current = battery->pdata->siop_max_charging_current_0;
			} else if (battery->siop_level <= 10) { /* 2 step(10) */
				max_charging_current = battery->pdata->siop_max_charging_current_10;
			} else {
				max_charging_current = battery->pdata->siop_max_charging_current_70; /* 1 step(70) */
			}
		}
		...
		if (is_nv_wireless_type(battery->cable_type)) {
			...
			if (*input_current > (battery->pdata->siop_max_input_power / battery->input_voltage))
...
static int sec_bat_set_charging_current(struct sec_battery_info *battery, int skip_afc_temp)
{
...
#endif
	if (is_hv_wire_type(battery->cable_type)) {
		input_current = battery->pdata->charging_current[battery->wire_status].input_current_limit;
		charging_current = battery->pdata->charging_current[battery->wire_status].fast_charging_current;
	}
	...
	if (is_nocharge_type(battery->cable_type)) {
		...
		if (!(battery->current_event & SEC_BAT_CURRENT_EVENT_SKIP_HEATING_CONTROL)) {
			...
			if (battery->pdata->chg_temp_check && !skip_afc_temp)
	...
	if (battery->topoff_current != topoff_current) {
		...
		psy_do_property(battery->pdata->charger_name, set,
			(enum power_supply_property) POWER_SUPPLY_EXT_PROP_CURRENT_FULL, value);
	...
	if (!afc_init) {
		...
		value.intval = 1;
		psy_do_property(battery->pdata->charger_name, set,
			(enum power_supply_property) POWER_SUPPLY_EXT_PROP_AFC_CHARGER_MODE,
			value);
#endif
	}
...
static int sec_bat_set_charge(
				...
				enum sec_battery_charging_mode chg_mode)
{
	union power_supply_propval val = {0, };
	ktime_t current_time = {0, };
	struct timespec ts = {0, };

	if (chg_mode < SEC_BATTERY_BUCKOFF)
		dev_info(battery->dev, "invalid chg_mode: %d", chg_mode);
	if ((battery->status == POWER_SUPPLY_STATUS_CHARGING || battery->status == POWER_SUPPLY_STATUS_FULL) &&
			chg_mode == SEC_BATTERY_BUCKOFF) {
		 dev_info(battery->dev, "invalid status: %d and chg_mode: %d", battery->status,  chg_mode);
	}
	if ((battery->status == POWER_SUPPLY_STATUS_NOT_CHARGING ||
			battery->status == POWER_SUPPLY_STATUS_DISCHARGING) &&
			chg_mode > SEC_BATTERY_CHARGING_NONE) {
		 dev_info(battery->dev, "invalid status: %d and chg_mode: %d", battery->status,  chg_mode);
	}
	...
	battery->charging_mode = chg_mode;
	switch (chg_mode){
	case SEC_BATTERY_BUCKOFF:
...
static bool sec_bat_check_recharge(struct sec_battery_info *battery)
{
	...
	if (battery->swelling_mode == SWELLING_MODE_CHARGING ||
		...
		if (battery->voltage_now < swelling_rechg_voltage &&
			   battery->charging_mode == SEC_BATTERY_CHARGING_NONE) {
	...
	if (battery->status == POWER_SUPPLY_STATUS_FULL &&
		...
		if (battery->current_event & SEC_BAT_CURRENT_EVENT_LOW_TEMP) {
			...
			if (battery->pdata->is_b2b_model)   /*for B2B Model , spec is float voltage - 200mV */ 
				recharging_voltage -= 50; 
				...
				struct sec_battery_info *battery)
{
	...
	int health = POWER_SUPPLY_HEALTH_GOOD;
	unsigned int pre_swell_sts = battery->current_event & SEC_BAT_CURRENT_EVENT_SWELLING_MODE;
	bool swell_chg = false;
	static int temp_chg_cnt = 0;
	...
	if (battery->pdata->usb_thermal_source) {
		if (battery->usb_temp >= battery->temp_highlimit_threshold ||
				(battery->usb_temp > battery->temp_highlimit_recovery
				 && battery->health == POWER_SUPPLY_HEALTH_OVERHEATLIMIT))
			health = POWER_SUPPLY_HEALTH_OVERHEATLIMIT;
	} else {
		if (temp_value >= battery->temp_highlimit_threshold ||
				(temp_value > battery->temp_highlimit_recovery
				 && battery->health == POWER_SUPPLY_HEALTH_OVERHEATLIMIT))
			health = POWER_SUPPLY_HEALTH_OVERHEATLIMIT;
	}
	if (health != POWER_SUPPLY_HEALTH_OVERHEATLIMIT) {
	...
	if (battery->skip_swelling) {
	...
	} else if (temp_value >= battery->pdata->swelling_high_temp_block ||
			(temp_value > battery->pdata->swelling_high_temp_recov &&
		...
		sec_bat_set_current_event(battery, SEC_BAT_CURRENT_EVENT_HIGH_TEMP_SWELLING,
				SEC_BAT_CURRENT_EVENT_SWELLING_MODE);
	} else if (temp_value <= battery->pdata->swelling_low_temp_block_2nd ||
		...
		sec_bat_set_current_event(battery, SEC_BAT_CURRENT_EVENT_LOW_TEMP_SWELLING,
				SEC_BAT_CURRENT_EVENT_SWELLING_MODE);
	} else if (temp_value <= battery->pdata->swelling_low_temp_block_1st ||
	...
	if (pre_swell_sts != (battery->current_event & SEC_BAT_CURRENT_EVENT_SWELLING_MODE)) {
		swell_chg = true;
#if defined(CONFIG_BATTERY_CISD)
	...
	dev_info(battery->dev, "%s, health %d, swelling_mode %d(0x%x)\n",__func__,
	...
	if (temp_chg_cnt >= battery->pdata->temp_check_count) {
		battery->health = health;
		temp_chg_cnt = 0;
	}
...
#if defined(CONFIG_BATTERY_SWELLING)
		if (swell_chg || battery->health_change) {
			union power_supply_propval val = {0, };
			int float_voltage = battery->pdata->swelling_normal_float_voltage;
			psy_do_property(battery->pdata->charger_name, get,
					POWER_SUPPLY_PROP_VOLTAGE_MAX, val);
			...
			if (battery->current_event & (SEC_BAT_CURRENT_EVENT_HIGH_TEMP_SWELLING |
						SEC_BAT_CURRENT_EVENT_LOW_TEMP_SWELLING)) {
				float_voltage = battery->pdata->swelling_drop_float_voltage;
				if (!battery->swelling_mode) {
					int swelling_rechg_voltage = battery->pdata->swelling_high_rechg_voltage;
					...
					if (battery->voltage_now > swelling_rechg_voltage) {
						pr_info("%s: charging disable(%dmV)\n", __func__, battery->voltage_now);
						sec_bat_set_charge(battery, SEC_BATTERY_CHARGING_NONE);
						battery->expired_time = battery->pdata->expired_time;
						battery->prev_safety_time = 0;
					}
...
#if defined(CONFIG_BATTERY_SWELLING)
	if (battery->swelling_mode != SWELLING_MODE_NONE)
		return true;
				...
				struct sec_battery_info *battery)
{
	...
	case POWER_SUPPLY_STATUS_FULL:
		if (battery->expired_time == 0) {
			...
			battery->is_recharging = false;
			if (sec_bat_set_charge(battery, SEC_BATTERY_CHARGING_NONE)) {
				dev_err(battery->dev,
				...
				struct sec_battery_info *battery)
{
	...
	case SEC_BATTERY_FULLCHARGED_ADC:
		...
		if (battery->current_adc < topoff_current) {
	...
	case SEC_BATTERY_FULLCHARGED_SOC:
		if (battery->capacity <= topoff_current) {
			battery->full_check_cnt++;
			dev_dbg(battery->dev,
				"%s: Full Check SOC (%d)\n",
				__func__,
				...
				struct sec_battery_info *battery)
{
	...
	if (battery->charging_mode == SEC_BATTERY_CHARGING_1ST &&
		...
		sec_bat_set_charging_current(battery, 0);
		sec_bat_set_charge(battery, SEC_BATTERY_CHARGING_2ND);	
	} else {
#if defined(CONFIG_BATTERY_CISD)
...
#endif
			pr_info("%s : swelling Full, capacity(%d)\n",
					__func__, battery->capacity);
			return true;
		}
#endif
				...
				struct sec_battery_info *battery)
{
	...
	battery->capacity = value.intval;
		...
		dev_info(battery->dev,
			"%s:Vnow(%dmV),Inow(%dmA),Imax(%dmA),Ichg(%dmA),SOC(%d%%),Tbat(%d),Tusb(%d),Tchg(%d),Twpc(%d),Tblkt(%d)"
			"\n", __func__,
...
bool sec_bat_hv_wc_normal_mode_check(struct sec_battery_info *battery)
{
	...
	psy_do_property(battery->pdata->charger_name, get,
			(enum power_supply_property) POWER_SUPPLY_EXT_PROP_CHARGE_OTG_CONTROL, value);
...
static void sec_bat_fw_update_work(struct sec_battery_info *battery, int mode)
{
	...
	switch (mode) {
		...
		case SEC_WIRELESS_RX_BUILT_IN_MODE:
			value.intval = mode;
			psy_do_property(battery->pdata->wireless_charger_name, set,
				(enum power_supply_property) POWER_SUPPLY_EXT_PROP_CHARGE_POWERED_OTG_CONTROL, value);
			break;
		case SEC_WIRELESS_TX_ON_MODE:
			value.intval = true;
			psy_do_property(battery->pdata->charger_name, set,
				(enum power_supply_property) POWER_SUPPLY_EXT_PROP_CHARGE_UNO_CONTROL, value);
...
skip_current_monitor:
	...
	if (!is_nocharge_type(battery->cable_type) && battery->store_mode) {
		...
		if (battery->capacity >= battery->pdata->store_mode_charging_max) {
			int chg_mode = battery->misc_event & (BATT_MISC_EVENT_UNDEFINED_RANGE_TYPE | BATT_MISC_EVENT_HICCUP_TYPE) ?
				SEC_BATTERY_BUCKOFF : SEC_BATTERY_CHARGING_NONE;
...
static void sec_bat_cable_work(struct work_struct *work)
{
	...
	if(is_slate_mode(battery)) {
		...
		sec_bat_set_charge(battery, SEC_BATTERY_BUCKOFF);
#if defined(CONFIG_MUIC_HV) || defined(CONFIG_SUPPORT_QC30)
		/* set cable_type before slate mode off */
	...
	if (!is_nocharge_type(prev_cable_type) && !is_nocharge_type(battery->cable_type))
		goto skip_update_battery_status;
#if defined(CONFIG_BATTERY_SWELLING)
	battery->swelling_mode = SWELLING_MODE_NONE;
	...
	if (is_nocharge_type(battery->cable_type) ||
		...
		if (sec_bat_set_charge(battery, SEC_BATTERY_CHARGING_1ST))
				goto end_of_cable_work;
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	case BATT_FILTER_CFG:
		{
			psy_do_property(battery->pdata->fuelgauge_name, get,
				(enum power_supply_property) POWER_SUPPLY_EXT_PROP_FILTER_CFG, value);
	...
	case BLKT_TEMP_ADC:
		...
		break;
	case BATT_VF_ADC:
		if(battery->pdata->battery_check_type == SEC_BATTERY_CHECK_ADC) {
			i += scnprintf(buf + i, PAGE_SIZE - i, "%d\n",
	...
	case PREV_BATTERY_DATA:
		{
			if (battery->enable_update_data)
				i += scnprintf(buf + i, PAGE_SIZE - i, "%d, %d, %d, %d\n",
					...
					battery->is_jig_on,
					(battery->charging_mode > SEC_BATTERY_CHARGING_NONE)?1:0);
		}
		break;
		...
		if (sscanf(buf, "%10d\n", &x) == 1) {
			if (battery->pdata->wpc_en) {
				if (x == 0) {
					...
					value.intval = 0;
					psy_do_property(battery->pdata->wireless_charger_name, set,
						(enum power_supply_property) POWER_SUPPLY_EXT_PROP_WC_CONTROL, value);
	...
	case HMT_TA_CONNECTED:
		if (sscanf(buf, "%10d\n", &x) == 1) {
			...
			if (x) {
				value.intval = false;
				psy_do_property(battery->pdata->charger_name, set,
						(enum power_supply_property) POWER_SUPPLY_EXT_PROP_CHARGE_OTG_CONTROL,
						value);
				dev_info(battery->dev,
...
#if defined(CONFIG_CCIC_NOTIFIER)
			dev_info(battery->dev,
					"%s: HMT_TA_CHARGE(%d), cable_type(%d), status(%d)\n",
					__func__, x, battery->cable_type, battery->status);

			/* do not charge off without cable type, since wdt could be expired */
			if(x) {
				/* clear this event every time in case wrong hmt control*/
...
static void sec_bat_set_rp_current(struct sec_battery_info *battery, int cable_type)
{
	...
	if (battery->status != POWER_SUPPLY_STATUS_DISCHARGING)
		sec_bat_check_input_voltage(battery);
	sec_bat_set_charging_current(battery, 1);
}
#endif
...
static int make_pd_list(struct sec_battery_info *battery)
{
	...
	value.intval = battery->pd_list.num_apdo > 0 ? 1 : 0;
	psy_do_property(battery->pdata->charger_name, set,
		(enum power_supply_property) POWER_SUPPLY_EXT_PROP_DIRECT_FIXED_PDO, value);
#endif
		...
		struct sec_battery_info *battery)
{
	...
	ret = of_property_read_u32(np, "battery,blkt_temp_check",
		&pdata->blkt_temp_check);
	if (ret)
		pr_info("%s : blkt_temp_check is Empty\n", __func__);
	...
	if (ret) {
		pr_info("%s : Temp adc channel is Empty\n", __func__);
		pdata->temp_adc_channel = 0x51; /* VADC_AMUX_THM5_PU2 */
	}

	ret = of_property_read_u32(np, "battery,chg_temp_adc_channel",
		&pdata->chg_temp_adc_channel);
	if (ret) {
		pr_info("%s : CHG Temp adc channel is Empty\n", __func__);
		pdata->chg_temp_adc_channel = 0x4e; /* VADC_AMUX_THM2_PU2 */
	}

	ret = of_property_read_u32(np, "battery,usb_temp_adc_channel",
		&pdata->usb_temp_adc_channel);
	if (ret) {
		pr_info("%s : USB Temp adc channel is Empty\n", __func__);
		pdata->usb_temp_adc_channel = 0x56; /* VADC_AMUX5_GPIO_PU2 */
	}

	ret = of_property_read_u32(np, "battery,blkt_temp_adc_channel",
		&pdata->blkt_temp_adc_channel);
	if (ret) {
		pr_info("%s : BLKT Temp adc channel is Empty\n", __func__);
		pdata->blkt_temp_adc_channel = 0x4d; /* VADC_AMUX_THM1_PU2 */
	}
	...
	pdata->is_b2b_model = of_property_read_bool(np,
	...
	ret = of_property_read_u32(np, "battery,siop_max_charging_current_0",
			&pdata->siop_max_charging_current_0);
	if (ret) {
		pr_err("%s: siop_max_charging_current_0 is Empty\n", __func__);
		pdata->siop_max_charging_current_0 = 0;
	}
	ret = of_property_read_u32(np, "battery,siop_max_charging_current_10",
			&pdata->siop_max_charging_current_10);
	if (ret) {
		pr_err("%s: siop_max_charging_current_10 is Empty\n", __func__);
		pdata->siop_max_charging_current_10 = 500;
	}
	ret = of_property_read_u32(np, "battery,siop_max_charging_current_70",
			&pdata->siop_max_charging_current_70);
	if (ret) {
		pr_err("%s: siop_max_charging_current_70 is Empty\n", __func__);
		pdata->siop_max_charging_current_70 = 1800;
	}
	ret = of_property_read_u32(np, "battery,siop_max_input_power",
			&pdata->siop_max_input_power);
	if (ret) {
		pr_err("%s: siop_max_input_power is Empty\n", __func__);
		pdata->siop_max_input_power = 6000;
	}
