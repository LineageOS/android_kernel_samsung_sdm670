...
static int32_t sm5705_fg_i2c_read_word(struct i2c_client *client, uint8_t reg_addr)
{
	uint16_t data = 0;
	int ret;
...
static unsigned int sm5705_get_vbat(struct i2c_client *client)
{
	...
	if ((fuelgauge->info.volt_alert_flag == true) && vbat > 3400) {
		...
		sm5705_fg_fuelalert_init(client, fuelgauge->pdata->fuel_alert_soc);
...
static bool sm5705_fg_reg_init(struct i2c_client *client, int is_surge)
{
	...
	pr_info("%s: RS_MIX_FACTOR = 0x%x, RS_MAX = 0x%x, RS_MIN = 0x%x, MIX_RATE = 0x%x, MIX_INIT_BLANK = 0x%x\n",
		__func__, fuelgauge->info.rs_value[2], fuelgauge->info.rs_value[3], fuelgauge->info.rs_value[4],
	...
	pr_info("%s: VOLT_CAL = 0x%x, curr_offset = 0x%x, p_curr_cal = 0x%x, n_curr_cal = 0x%x\n",
		__func__,fuelgauge->info.volt_cal, fuelgauge->info.curr_offset, fuelgauge->info.p_curr_cal, fuelgauge->info.n_curr_cal);
	// MISC write
...
static bool sm5705_fg_init(struct i2c_client *client, bool is_surge)
{
	...
	pr_info("%s: q_max_now = 0x%x\n", __func__, fuelgauge->info.q_max_now);
#endif
	ret = sm5705_fg_i2c_read_word(client, SM5705_REG_PARAM_CTRL);
	pr_info("%s: SM5705_REG_PARAM_CTRL 0x13 = 0x%x \n", __func__, ret);
	if (ret != (SM5705_FG_PARAM_LOCK_CODE | SM5705_FG_TABLE_LEN)) {
		pr_info("%s: SM5705_FG_PARAM_LOCK_CODE is abnormal Start quick-start\n", __func__);
		// SW reset code
		sm5705_fg_i2c_verified_write_word(client, SM5705_REG_RESET, SW_RESET_CODE);
		// delay 800ms
		msleep(800);
	}
	if(sm5705_fg_check_reg_init_need(client) || (ret != (SM5705_FG_PARAM_LOCK_CODE | SM5705_FG_TABLE_LEN)))
	...
	pr_info("%s: vbat=%d, vbat_avg=%d, curr=%d, curr_avg=%d, ocv=%d, temp=%d, cycle=%d, soc=%d, state=0x%x, Q=0x%x\n",
...
static int sm5705_abnormal_reset_check(struct i2c_client *client)
{
	...
	if(sm5705_fg_check_reg_init_need(client) || (reset_read == 0)) {
...
void sm5705_vbatocv_check(struct i2c_client *client)
{
	...
	ret = sm5705_fg_i2c_read_word(client, SM5705_REG_PARAM_RUN_UPDATE);
...
static int sm5705_cal_carc (struct i2c_client *client)
{
	...
	pr_info("%s: <%d %d %d %d %d %d %d %d %d %d>, temp_fg = %d ,p_curr_cal = 0x%x, n_curr_cal = 0x%x, "
		...
		__func__, fuelgauge->info.en_high_fg_temp_cal,
		...
		fuelgauge->info.temp_fg, p_curr_cal, n_curr_cal, temp_curr_offset, volt_cal, fg_delta_volcal);
	...
	pr_info("%s: <%d %d %d %d %d %d %d %d %d %d>, p_curr_cal = 0x%x, n_curr_cal = 0x%x, mix_factor=0x%x ,batt_temp = %d\n",
		__func__, fuelgauge->info.en_high_temp_cal,
...
static int sm5705_get_all_value(struct i2c_client *client)
{
	...
	psy_do_property("sm5705-charger", get, POWER_SUPPLY_PROP_HEALTH, value);
#ifdef SM5705_FG_FULL_DEBUG
	pr_info("%s: get POWER_SUPPLY_PROP_HEALTH = 0x%x\n", __func__, value.intval);
#endif
	fuelgauge->info.flag_charge_health = (value.intval == POWER_SUPPLY_HEALTH_GOOD) ? 1 : 0;
	fuelgauge->is_charging = (fuelgauge->info.flag_charge_health | fuelgauge->ta_exist)
					&& (fuelgauge->info.batt_current>=30);
	// check charger status
	psy_do_property("sm5705-charger", get, POWER_SUPPLY_PROP_STATUS, value);
...
static int sm5705_fg_parse_dt(struct sec_fuelgauge_info *fuelgauge)
{
	...
	if (np == NULL)
		...
		pr_info("%s: capacity_max: %d, capacity_max_margin: 0x%x, capacity_min: %d\n",
	...
	PINFO("%s = <0x%x 0x%x 0x%x 0x%x 0x%x>\n", prop_name,
		rs_value[0], rs_value[1], rs_value[2], rs_value[3], rs_value[4]);
	...
	PINFO("%s = <%d, 0x%x, 0x%x, 0x%x>\n", prop_name,
		fuelgauge->info.curr_lcal_en, fuelgauge->info.curr_lcal_0,
		fuelgauge->info.curr_lcal_1, fuelgauge->info.curr_lcal_2);
	...
	PINFO("%s = <%d, %d, %d>\n", prop_name,
		fuelgauge->info.en_fg_temp_volcal, fuelgauge->info.fg_temp_volcal_denom,
		fuelgauge->info.fg_temp_volcal_fact);
...
static void sm5705_fg_get_scaled_capacity(struct sec_fuelgauge_info *fuelgauge,
...
/* capacity is integer */
static void sm5705_fg_get_atomic_capacity(struct sec_fuelgauge_info *fuelgauge,
						union power_supply_propval *val)
{
	...
	if (fuelgauge->pdata->capacity_calculation_type &
		...
		if (!fuelgauge->ta_exist && fuelgauge->capacity_old < val->intval) {
...
static int sm5705_fg_check_capacity_max(struct sec_fuelgauge_info *fuelgauge, int capacity_max)
{
	int cap_max, cap_min;
...
static int sm5705_fg_calculate_dynamic_scale(struct sec_fuelgauge_info *fuelgauge, int capacity)
{
	union power_supply_propval raw_soc_val;

	raw_soc_val.intval = sm5705_get_soc(fuelgauge->client);
	if (raw_soc_val.intval < fuelgauge->pdata->capacity_max - fuelgauge->pdata->capacity_max_margin) {
		...
		fuelgauge->capacity_max = 
			...
			(fuelgauge->pdata->capacity_max + fuelgauge->pdata->capacity_max_margin) :
			raw_soc_val.intval;
       		pr_debug("%s: raw soc (%d)", __func__, fuelgauge->capacity_max);
	}
	fuelgauge->capacity_max = (fuelgauge->capacity_max * 100 / (capacity + 1));
...
static int sm5705_fg_set_property(struct power_supply *psy, enum power_supply_property psp,
...
static irqreturn_t sm5705_fg_irq_thread(int irq, void *irq_data)
{
	...
	if (fuelgauge->pdata->fuel_alert_soc >= 0) {
		fuel_alerted = sm5705_fg_is_fuelalerted(fuelgauge->client);
		...
		if (fuel_alerted == fuelgauge->is_fuel_alerted) {
			if (!fuelgauge->pdata->repeated_fuelalert) {
				dev_dbg(&fuelgauge->client->dev, "%s: Fuel-alert Repeated (%d)\n",
				...
				struct device_attribute *attr, char *buf)
{
	...
	case FG_DATA:
		...
		for (j = 0; j < 12; j++) {
		    ...
		    reg_data = sm5705_fg_i2c_read_word(fuelgauge->client, reg + j * 0x10);
	...
	case FG_DATA:
		if (sscanf(buf, "0x%8x 0x%8x", &x, &y) == 2) {
			if (x >= 0x00 && x <= 0xff) {
				...
				if (sm5705_fg_i2c_write_word(fuelgauge->client, addr, data)) {
...
static const struct power_supply_desc sm5705_fuelgauge_power_supply_desc = {
	.name = "sm5705-fuelgauge",
	.type = POWER_SUPPLY_TYPE_UNKNOWN,
	.properties = sm5705_fuelgauge_props,
	.num_properties = ARRAY_SIZE(sm5705_fuelgauge_props),
	.get_property = sm5705_fg_get_property,
	.set_property = sm5705_fg_set_property,
};
...
//	struct battery_data_t *battery_data = NULL;
	...
	if (client->dev.of_node) {
		...
		if (error < 0) {
			dev_err(&client->dev, "%s: Failed to get fuel_int\n", __func__);
			goto err_parse_dt;
		}
	} else	{
		dev_err(&client->dev, "%s: Failed to get of_node\n", __func__);
	...
	if (!sm5705_fg_init(fuelgauge->client, false)) {
		dev_err(&client->dev, "%s: Failed to Initialize Fuelgauge\n", __func__);
		goto err_devm_free;
	}
	...
	fuelgauge->psy_fg = power_supply_register(&client->dev, 
		&sm5705_fuelgauge_power_supply_desc, &fuelgauge_cfg);
	if (!fuelgauge->psy_fg) {
	...
	fuelgauge->is_fuel_alerted = false;
	if (fuelgauge->pdata->fuel_alert_soc >= 0) {
		if (sm5705_fg_fuelalert_init(fuelgauge->client, fuelgauge->pdata->fuel_alert_soc))
			wake_lock_init(&fuelgauge->fuel_alert_wake_lock,
				WAKE_LOCK_SUSPEND, "fuel_alerted");
		else {
			dev_err(&client->dev, "%s: Failed to Initialize Fuel-alert\n", __func__);
			goto err_irq;
		}
	}

	if (fuelgauge->pdata->fg_irq > 0) {
		INIT_DELAYED_WORK(&fuelgauge->isr_work, sm5705_fg_isr_work);
		...
		if (fuelgauge->fg_irq > 0) {
			...
			if (ret) {
				dev_err(&client->dev, "%s: Failed to Reqeust IRQ\n", __func__);
	...
	ret = sm5705_create_attrs(&client->dev);
	if (ret) {
		dev_err(&client->dev, "%s : Failed to create_attrs\n", __func__);
		goto err_irq;
	}

	pr_info("%s: SEC Fuelgauge Driver Loaded\n", __func__);
...
static int sm5705_fuelgauge_remove(struct i2c_client *client)
{
	struct sec_fuelgauge_info *fuelgauge = i2c_get_clientdata(client);
