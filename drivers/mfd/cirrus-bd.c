...
void cirrus_bd_store_values(const char *mfd_suffix)
{
	...
	dev_info(cirrus_bd->dev, "Max Excursion:\t\t%d.%d\n",
	...
	dev_info(cirrus_bd->dev, "Max Temp:\t\t\t%d.%d\n",
...
static char attr_names_prealloc[CIRRUS_MAX_AMPS][CIRRUS_BD_NUM_ATTRS_AMP][20];
							...
							int index)
{
	...
	for (i = 0; i < CIRRUS_BD_NUM_ATTRS_AMP; i++) {
					...
					const char **mfd_suffixes)
{
	...
	cirrus_bd_attr_grp.attrs = kzalloc(sizeof(struct attribute *) *
					...
					CIRRUS_BD_NUM_ATTRS_BASE + 1),
							GFP_KERNEL);
	for (i = 0; i < num_amps; i++) {
		new_attrs = cirrus_bd_create_amp_attrs(mfd_suffixes[i], i);
