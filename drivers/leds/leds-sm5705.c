...
struct msm_pinctrl_info *g_led_pinctrl_info  = NULL;
...
static inline unsigned char _calc_flash_current_offset_to_mA(int index, unsigned short current_mA)
{
	if (index) { // FLED1
		...
		return current_mA < 700 ? (((current_mA - 30) / 25) & 0x1F) : ((((current_mA - 700) / 50) + 0xF) & 0x1F);
	}
}
...
static int sm5705_FLEDx_set_flash_current(struct sm5705_fled_info *sm5705_fled, int index, unsigned short current_mA)
{
	...
	pr_info("%s: set flash current!!  %d mA for index: %d\n", __func__, current_mA, index);

	reg_val = _calc_flash_current_offset_to_mA(index, current_mA);
	pr_info("%s: reg_val:%d is set -  %d mA for index: %d\n", __func__, reg_val, current_mA, index);
	...
	/*
	 * Revisit: FLED1CNTL3 write not reflecting right status
	 */
	ret = sm5705_read_reg(sm5705_fled->i2c, SM5705_REG_FLED1CNTL3 + (index * 4), &read_val);
	if (IS_ERR_VALUE(ret)) {
		dev_err(sm5705_fled->dev, "%s: fail to read REG:FLED%dCNTL3 (value=%d)\n", __func__, index, read_val);
		return ret;
	}
	pr_info("%s: read value:%d  %d mA for index: %d\n", __func__, read_val, current_mA, index);
...
static int sm5705_fled_turn_on_torch(struct sm5705_fled_info *sm5705_fled, int index, unsigned short current_mA)
{
	...
	pr_debug("sm5705-fled : %s fimc_is_activated=%d, current_mA=%d\n", __func__, fimc_is_activated,current_mA);
...
{
	if (assistive_light[index] == false) {
		dev_info(g_sm5705_fled->dev, "%s: Preflash - ON,pre_flash_current_mA = %d(mA)\n", __func__,pre_flash_current_mA);
...
{
	if (assistive_light[index] == false) {
		dev_info(g_sm5705_fled->dev, "%s: Flash setted by user- ON, flash_current_mA = %d(mA)\n", __func__,flash_current_mA);
...
static ssize_t sm5705_rear_flash_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	...
	case 200:
		...
		dev_info(dev, "%s: muic_torch_prepare success!!  %d mA\n", __func__, value_u32);
