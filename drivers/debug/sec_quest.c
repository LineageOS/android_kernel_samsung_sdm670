...
static int qdaf_cur_cmd_mode;  /* this value need to be defined here for lower version kernel build_error*/


static void print_param_quest_data(void)
{
...
void set_ddr_scan_result_for_smd(void)
{
	int ddr_result = DDRTEST_INCOMPLETED;
...
static void do_quest(void)
{
	...
	switch (quest_step) {
		case SMD_QUEST:
			...
			snprintf(log_path, 50, "logPath:%s%c", SMD_QUEST_LOGPATH, '\0'); 
			argv[1] = log_path;
			QUEST_PRINT("SMD_QUEST, quest_step : %d", quest_step);
			break;
		case CAL_QUEST:
			argv[0] = CAL_QUEST_PROG;
			snprintf(log_path, 50, "logPath:%s%c", CAL_QUEST_LOGPATH, '\0'); 
			argv[1] = log_path;
			argv[2] = "Reboot";
		...
		case MAIN_QUEST:
			...
			snprintf(log_path, 50, "logPath:%s%c", MAIN_QUEST_LOGPATH, '\0'); 
			argv[1] = log_path;
			argv[2] = "Reboot";		
...
static int get_quest_uefi_result(char* uefi_log_path, int will_print)
{
	...
	snprintf(questresult_file, 50, "%s/%s%c", uefi_log_path, "questresult.txt", '\0'); 
	...
	switch (total_result) {
		...
		default: {
			if( param_quest_data.quest_step == CAL_QUEST ) {
				...
				snprintf(options, 50, "action:lastkmsg output_log_path:%s%c", CAL_QUEST_LOGPATH, '\0'); 
	...
	switch (hlos_result) {
		...
		case TOTALTEST_NO_RESULT_FILE: {
			if (quest_step == SMD_QUEST) {
				...
				if( first_check ) {
					snprintf(options, 50, "action:lastkmsg output_log_path:%s%c", SMD_QUEST_LOGPATH, '\0'); 
					make_debugging_logs(options);
				}
...
static ssize_t show_quest_support(struct device *dev,
				struct device_attribute *attr, char *buf)
{
#if defined(CONFIG_ARCH_MSM8998) || defined(CONFIG_ARCH_MSM8996) || defined(CONFIG_ARCH_SDM845) || defined(CONFIG_ARCH_SM8150) || defined(CONFIG_ARCH_SDM670)
				     ...
				     struct device_attribute *attr, char *buf)
{
	...
	switch( uefi_result ) {
		...
		case TOTALTEST_NO_RESULT_STRING : {
			...
			snprintf(options, 50, "action:lastkmsg output_log_path:%s%c", MAIN_QUEST_LOGPATH, '\0'); 
			make_debugging_logs(options);			
			break;
		}
		case TOTALTEST_FAIL : {
				     ...
				     struct device_attribute *attr, char *buf)
{
	...
	#endif
	#if defined(CONFIG_ARCH_SDM710) || defined(CONFIG_ARCH_SDM670)
		return snprintf(buf, BUFF_SZ, "SDM710.1004.01.RELEASE\n");
	#else		
