...
int cam_sensor_check_resolution(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	if (iris_cam_year < (uint32_t)'0') iris_cam_year = '0';
	CAM_INFO(CAM_SENSOR, "read year : %c", (char)iris_cam_year);
	...
	if (iris_cam_month < (uint32_t)'0') iris_cam_month = '0';
	CAM_INFO(CAM_SENSOR, "read month : %c", (char)iris_cam_month);
	...
	if (iris_cam_company < (uint32_t)'0') iris_cam_company = '0';
	CAM_INFO(CAM_SENSOR, "read company : %c", (char)iris_cam_company);
	...
	struct i2c_settings_list *i2c_list)
{
	...
	if (i2c_list->op_code == CAM_SENSOR_I2C_WRITE_RANDOM) {
		...
		if (i2c_list->i2c_settings.reg_setting[0].reg_addr == STREAM_ON_ADDR
			&& i2c_list->i2c_settings.reg_setting[0].reg_data == 0x0 && s_ctrl->sensordata->slave_info.sensor_id == RETENTION_SENSOR_ID) {
			uint32_t frame_cnt = 0;
			int retry_cnt = 10;
	...
	void *arg)
{
	...
	case CAM_SENSOR_PROBE_CMD: {
		...
		if (rc < 0) {
			CAM_ERR(CAM_SENSOR,
				"Probe Fail - slot:%d,slave_addr:0x%x,sensor_id:0x%x",
				s_ctrl->soc_info.index,
				s_ctrl->sensordata->slave_info.sensor_slave_addr,
				s_ctrl->sensordata->slave_info.sensor_id);
		} else {
...
int cam_sensor_power_up(struct cam_sensor_ctrl_t *s_ctrl)
{
	...
	CAM_INFO(CAM_SENSOR, "cam sensor power up slot:0x%x sensor_id:0x%x slave_addr:0x%x",
			soc_info->index,  s_ctrl->sensordata->slave_info.sensor_id,
			s_ctrl->sensordata->slave_info.sensor_slave_addr);
