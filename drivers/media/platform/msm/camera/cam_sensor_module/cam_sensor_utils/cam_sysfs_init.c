...
#elif defined(CONFIG_SEC_GTACTIVEXL_PROJECT)
    char cam_type_lsi[] = "SLSI_S5K3L6\n";
#else
	char cam_type_lsi[] = "SLSI_S5K3M5SX\n";
#endif
	char cam_type_sony[] = "SONY_IMX576\n";
...
static ssize_t back_camera3_firmware_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	pr_info("[FW_DBG] cam3_fw_ver : %s\n", cam3_fw_ver);
	return snprintf(buf, sizeof(cam3_fw_ver), "%s", cam3_fw_ver);
}

static ssize_t back_camera3_firmware_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	pr_info("[FW_DBG] buf : %s\n", buf);
	snprintf(cam3_fw_ver, sizeof(cam3_fw_ver), "%s", buf);

	return size;
}
char cam3_fw_full_ver[SYSFS_FW_VER_SIZE] = "NULL NULL NULL\n";//multi module
static ssize_t back_camera3_firmware_full_show(struct device *dev,
					 struct device_attribute *attr, char *buf)
{
	pr_info("[FW_DBG] cam3_fw_ver : %s\n", cam3_fw_full_ver);
	return snprintf(buf, sizeof(cam3_fw_full_ver), "%s", cam3_fw_full_ver);
}

static ssize_t back_camera3_firmware_full_store(struct device *dev,
					  struct device_attribute *attr, const char *buf, size_t size)
{
	pr_info("[FW_DBG] buf : %s\n", buf);
	snprintf(cam3_fw_full_ver, sizeof(cam3_fw_full_ver), "%s", buf);
...
static ssize_t back_camera3_afcal_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	char tempbuf[10];
	char N[] = "N ";
...
{
	pr_info("[FW_DBG] buf : %s\n", buf);
//	snprintf(rear3_cam_info, sizeof(rear3_cam_info), "%s", buf);
					 ...
					 struct device_attribute *attr, char *buf)
{
	...
	ret = memcpy(buf, rear3_sensor_id, sizeof(rear3_sensor_id));
	if (ret)
		return sizeof(rear3_sensor_id);
	return 0;
}
...
{
	pr_info("[FW_DBG] buf : %s\n", buf);
//	snprintf(rear3_sensor_id, sizeof(rear3_sensor_id), "%s", buf);
...
static ssize_t rear2_dual_cal_extra_show(struct device *dev,
					 struct device_attribute *attr, char *buf)
{
	void *ret = NULL;
	int copy_size = 0;

	pr_info("[FW_DBG] rear2_dual_cal_extra : %s\n", rear2_dual_cal);

	if (FROM_REAR2_DUAL_CAL_SIZE > SYSFS_MAX_READ_SIZE)
        {
            copy_size = FROM_REAR2_DUAL_CAL_SIZE - SYSFS_MAX_READ_SIZE;

	    ret = memcpy(buf, &rear2_dual_cal[SYSFS_MAX_READ_SIZE], copy_size);
...
#ifndef CONFIG_DISABLE_OIS_SDM710
		...
		struct device_attribute *attr, char *buf)
{
	...
	if (ec_param != NULL) {
		...
		switch (moduelid_chk) {
		case HW_PARAMS_MI_VALID:
			rc = sprintf(buf, "\"CAMIR3_ID\":\"%c%c%c%c%cXX%02X%02X%02X\",\"I2CR2_AF\":\"%d\",\"I2CR2_COM\":\"%d\",\"I2CR2_OIS\":\"%d\","
					"\"I2CR2_SEN\":\"%d\",\"MIPIR2_COM\":\"%d\",\"MIPIR2_SEN\":\"%d\"\n",
					rear_module_id[0], rear_module_id[1], rear_module_id[2], rear_module_id[3],
					rear_module_id[4], rear_module_id[7], rear_module_id[8], rear_module_id[9],
					ec_param->i2c_af_err_cnt, ec_param->i2c_comp_err_cnt, ec_param->i2c_ois_err_cnt,
		...
		case HW_PARAMS_MIR_ERR_1:
			rc = sprintf(buf, "\"CAMIR3_ID\":\"MIR_ERR\",\"I2CR2_AF\":\"%d\",\"I2CR2_COM\":\"%d\",\"I2CR2_OIS\":\"%d\","
					"\"I2CR2_SEN\":\"%d\",\"MIPIR2_COM\":\"%d\",\"MIPIR2_SEN\":\"%d\"\n",
					ec_param->i2c_af_err_cnt, ec_param->i2c_comp_err_cnt, ec_param->i2c_ois_err_cnt,
					ec_param->i2c_sensor_err_cnt, ec_param->mipi_comp_err_cnt,
					ec_param->mipi_sensor_err_cnt);
		...
		default:
			rc = sprintf(buf, "\"CAMIR3_ID\":\"MI_NO\",\"I2CR2_AF\":\"%d\",\"I2CR2_COM\":\"%d\",\"I2CR2_OIS\":\"%d\","
					"\"I2CR2_SEN\":\"%d\",\"MIPIR2_COM\":\"%d\",\"MIPIR2_SEN\":\"%d\"\n",
					ec_param->i2c_af_err_cnt, ec_param->i2c_comp_err_cnt, ec_param->i2c_ois_err_cnt,
					ec_param->i2c_sensor_err_cnt, ec_param->mipi_comp_err_cnt,
					ec_param->mipi_sensor_err_cnt);
...
//FACTORY
#if defined(CONFIG_SEC_A8SQLTE_PROJECT)
char strCameraIds[40] = "0 1 20 21 50 52\n";
#else
char strCameraIds[40] = "0 1 \n";
#endif
...
static DEVICE_ATTR(rear2_dualcal_extra, S_IRUGO, rear2_dual_cal_extra_show, NULL);
...
static DEVICE_ATTR(rear3_camfw, S_IRUGO|S_IWUSR|S_IWGRP,
	back_camera3_firmware_show, back_camera3_firmware_store);
static DEVICE_ATTR(rear3_camfw_full, S_IRUGO|S_IWUSR|S_IWGRP,
	back_camera3_firmware_full_show, back_camera3_firmware_full_store);
static DEVICE_ATTR(rear3_afcal, S_IRUGO, back_camera3_afcal_show, NULL);
static DEVICE_ATTR(rear3_caminfo, S_IRUGO|S_IWUSR|S_IWGRP,
		rear3_camera_info_show, rear3_camera_info_store);
static DEVICE_ATTR(rear3_camtype, S_IRUGO, back_camera3_type_show, NULL);// 5e9 added
...
static DEVICE_ATTR(rear3_moduleid, S_IRUGO,
		rear3_moduleid_show, NULL);
...
static int __init cam_sysfs_init(void)
{
	...
	if (device_create_file(cam_dev_back, &dev_attr_rear3_afcal) < 0) {
		pr_err("Failed to create device file!(%s)!\n",
			dev_attr_rear3_afcal.attr.name);
		ret = -ENODEV;
	}

	if (device_create_file(cam_dev_back, &dev_attr_rear3_tilt) < 0) {
		pr_err("Failed to create device file!(%s)!\n",
	...
	if (device_create_file(cam_dev_back, &dev_attr_rear3_sensorid_exif) < 0) {
		pr_err("Failed to create device file!(%s)!\n",
	...
	if (device_create_file(cam_dev_back, &dev_attr_rear2_dualcal) < 0) {
		pr_err("Failed to create device file!(%s)!\n",
			dev_attr_rear2_dualcal.attr.name);
		ret = -ENODEV;
	}
	if (device_create_file(cam_dev_back, &dev_attr_rear2_dualcal_extra) < 0) {
		pr_err("Failed to create device file!(%s)!\n",
			dev_attr_rear2_dualcal_extra.attr.name);
		ret = -ENODEV;
	}
	if (device_create_file(cam_dev_back, &dev_attr_rear2_dualcal_size) < 0) {
		pr_err("Failed to create device file!(%s)!\n",
			dev_attr_rear2_dualcal_size.attr.name);
		ret = -ENODEV;
	}
	if (device_create_file(cam_dev_back, &dev_attr_rear3_dualcal) < 0) {
		pr_err("Failed to create device file!(%s)!\n",
			dev_attr_rear3_dualcal.attr.name);
		ret = -ENODEV;
	}
	if (device_create_file(cam_dev_back, &dev_attr_rear3_dualcal_extra) < 0) {
		pr_err("Failed to create device file!(%s)!\n",
			dev_attr_rear3_dualcal_extra.attr.name);
		ret = -ENODEV;
	}
	if (device_create_file(cam_dev_back, &dev_attr_rear3_dualcal_size) < 0) {
		pr_err("Failed to create device file!(%s)!\n",
