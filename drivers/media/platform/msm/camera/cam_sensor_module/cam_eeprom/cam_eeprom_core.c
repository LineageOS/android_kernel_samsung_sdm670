...
#define REAR3_CAL_INFO_LOAD_FIX
//#define CAM_EEPROM_DBG  1
//#define CAM_EEPROM_DBG_DUMP  1

struct cam_sensor_i2c_reg_setting load_otp_setfile = {
	load_s5k5e9yx_otp_setfile_reg, sizeof(load_s5k5e9yx_otp_setfile_reg)/sizeof(struct cam_sensor_i2c_reg_array), CAMERA_SENSOR_I2C_TYPE_WORD, CAMERA_SENSOR_I2C_TYPE_BYTE, 50
};

struct cam_sensor_i2c_reg_setting init_read_otp = {
#ifdef CONFIG_S5K5E9YX
	init_read_s5k5e9yx_otp_reg, sizeof(init_read_s5k5e9yx_otp_reg)/sizeof(struct cam_sensor_i2c_reg_array), CAMERA_SENSOR_I2C_TYPE_WORD, CAMERA_SENSOR_I2C_TYPE_BYTE, 10
#endif
};

struct cam_sensor_i2c_reg_setting finish_read_otp = {
#ifdef CONFIG_S5K5E9YX
	finish_read_s5k5e9yx_otp_reg, sizeof(finish_read_s5k5e9yx_otp_reg)/sizeof(struct cam_sensor_i2c_reg_array), CAMERA_SENSOR_I2C_TYPE_WORD, CAMERA_SENSOR_I2C_TYPE_BYTE, 10
#endif
};

struct cam_sensor_i2c_reg_setting init_write_otp = {
#ifdef CONFIG_S5K5E9YX
	init_write_s5k5e9yx_otp_reg, sizeof(init_write_s5k5e9yx_otp_reg)/sizeof(struct cam_sensor_i2c_reg_array), CAMERA_SENSOR_I2C_TYPE_WORD, CAMERA_SENSOR_I2C_TYPE_BYTE, 10
#endif
};

struct cam_sensor_i2c_reg_setting finish_write_otp = {
#ifdef CONFIG_S5K5E9YX
	finish_write_s5k5e9yx_otp_reg, sizeof(finish_write_s5k5e9yx_otp_reg)/sizeof(struct cam_sensor_i2c_reg_array), CAMERA_SENSOR_I2C_TYPE_WORD, CAMERA_SENSOR_I2C_TYPE_BYTE, 10
#endif
};
...
char hw_phone_info_tele[HW_INFO_MAX_SIZE] = HW_INFO_TELE;
char sw_phone_info_tele[SW_INFO_MAX_SIZE] = SW_INFO_TELE;
char vendor_phone_info_tele[VENDOR_INFO_MAX_SIZE] = VENDOR_INFO_TELE;
char process_phone_info_tele[PROCESS_INFO_MAX_SIZE] = PROCESS_INFO_TELE;
/*front */
...
/* depth */
char hw_phone_info_depth[HW_INFO_MAX_SIZE] = HW_INFO_DEPTH;
char sw_phone_info_depth[SW_INFO_MAX_SIZE] = SW_INFO_DEPTH;
char vendor_phone_info_depth[VENDOR_INFO_MAX_SIZE] = VENDOR_INFO_DEPTH;
char process_phone_info_depth[PROCESS_INFO_MAX_SIZE] = PROCESS_INFO_DEPTH;
...
//	uint8_t data[4] = { 0, };
	...
	if (e_ctrl->soc_info.index == CAM_EEPROM_IDX_FRONT) {// Front cam
		...
		else if (e_ctrl->soc_info.index == CAM_EEPROM_IDX_DEPTH) {
		...
		else if (e_ctrl->soc_info.index == CAM_EEPROM_IDX_TELE) {
	...
	if (e_ctrl->soc_info.index == CAM_EEPROM_IDX_FRONT) { //Front Cam
...
#if defined(CONFIG_SAMSUNG_MULTI_CAMERA)
		else if (e_ctrl->soc_info.index == CAM_EEPROM_IDX_TELE) { //Tele Sensor
		/* rear3 sensor id */
...
#endif
			CAM_DBG(CAM_EEPROM, "rear3_af_cal[0] macro: %d, rear3_af_cal[5] macro: %d, rear3_af_cal[9] pan: %d", rear3_af_cal[0], rear3_af_cal[5], rear3_af_cal[9]);
...
#ifdef CAM_EEPROM_DBG
			CAM_INFO(CAM_EEPROM, "rear3_module_id = %c %c %c %c %c %02X %02X %02X %02X %02X",
				rear3_module_id[0], rear3_module_id[1], rear3_module_id[2], rear3_module_id[3], rear3_module_id[4],
				rear3_module_id[5], rear3_module_id[6], rear3_module_id[7], rear3_module_id[8], rear3_module_id[9]);
#endif
		...
		/* rear3 manufacturer info */
			...
			CAM_DBG(CAM_EEPROM,
				...
				rear3_fw_ver[10]);
		/* temp phone version */
			snprintf(rear3_phone_fw_ver, FROM_MODULE_FW_INFO_SIZE+1, "%s%s%s%s", hw_phone_info_tele, sw_phone_info_tele, vendor_phone_info_tele, process_phone_info_tele);
		...
		/* temp load version */
			...
			normal_is_supported = CAMERA_CAL_CRC_TELE;
			normal_cri_rev = CRITERION_REV_TELE;
...
#if defined(CONFIG_SAMSUNG_MULTI_CAMERA)
		else if (e_ctrl->soc_info.index == CAM_EEPROM_IDX_DEPTH) { // Depth Sensor
		/* rear2 sensor id */
			memcpy(rear2_sensor_id, &e_ctrl->cal_data.mapdata[S5K5E9_OTP_SENSOR_ID_OFFSET], FROM_SENSOR_ID_SIZE);
		...
		/* rear2 module id */
			memcpy(rear2_module_id, &e_ctrl->cal_data.mapdata[S5K5E9_OTP_MODULE_ID_OFFSET], FROM_MODULE_ID_SIZE);
		...
		/*rear2 af cal*/
#if  defined(FROM_REAR_AF_CAL_MACRO_ADDR)
			memcpy(&rear2_af_cal[0], &e_ctrl->cal_data.mapdata[FROM_REAR_AF_CAL_MACRO_ADDR], 4);
#endif
#if  defined(FROM_REAR_AF_CAL_PAN_ADDR)
			memcpy(&rear2_af_cal[9], &e_ctrl->cal_data.mapdata[FROM_REAR_AF_CAL_PAN_ADDR], 4);
#endif
			CAM_DBG(CAM_EEPROM, "rear2_af_cal[0] macro: %d, rear2_af_cal[9] pan: %d", rear2_af_cal[0], rear2_af_cal[9]);
		...
		/* rear2 manufacturer info */
			...
			CAM_INFO(CAM_EEPROM,
				...
				rear2_fw_ver[10]);
		/* temp phone version */
			snprintf(rear2_phone_fw_ver, FROM_MODULE_FW_INFO_SIZE+1, "%s%s%s%s", hw_phone_info_depth, sw_phone_info_depth, vendor_phone_info_depth, process_phone_info_depth);
		...
		/* temp load version */
			...
			normal_is_supported = CAMERA_CAL_CRC_DEPTH;
			normal_cri_rev = CRITERION_REV_DEPTH;
		...
		/* rear2 dual cal */
			...
			CAM_DBG(CAM_EEPROM, "rear2 dual cal = %s", rear2_dual_cal);
	...
	if (e_ctrl->soc_info.index == CAM_EEPROM_IDX_WIDE)
		...
		rc = cam_eeprom_check_firmware_cal(e_ctrl->is_supported, map_ver, CAM_EEPROM_IDX_DEPTH);
	else if (e_ctrl->soc_info.index == CAM_EEPROM_IDX_TELE)
		rc = cam_eeprom_check_firmware_cal(e_ctrl->is_supported, map_ver, CAM_EEPROM_IDX_TELE);
	else {
	...
	const char *update_fw_ver, cam_eeprom_fw_version_idx update_fw_index, cam_eeprom_idx_type idx)
{
	...
	} else if (idx == CAM_EEPROM_IDX_DEPTH) {
...
int32_t cam_eeprom_check_firmware_cal(uint32_t camera_cal_crc, uint8_t cal_map_version, cam_eeprom_idx_type idx)
{
	...
	} else if (idx == CAM_EEPROM_IDX_TELE) {
	...
	if (idx == CAM_EEPROM_IDX_WIDE || idx == CAM_EEPROM_IDX_DEPTH || idx == CAM_EEPROM_IDX_TELE) {
		if (camera_fw_crc == OK)
			camera_fw_ack = OK;
		else {
			camera_fw_ack = CRASH;
	...
	if (idx == CAM_EEPROM_IDX_WIDE || idx == CAM_EEPROM_IDX_DEPTH || idx == CAM_EEPROM_IDX_TELE) {
		if (camera_cal_crc == CAMERA_CAL_CRC || camera_cal_crc == CAMERA_CAL_CRC_TELE || camera_cal_crc == CAMERA_CAL_CRC_DEPTH) {
			camera_cal_ack = OK;
...
#if 1
{
	...
	else if (subdev_id == CAM_EEPROM_IDX_DEPTH)
		map_ver = data->mapdata[REAR2_CAM_MAP_VERSION_ADDR];
	else if (subdev_id == CAM_EEPROM_IDX_TELE)
	...
	if(0/*subdev_id != 1 && data->mapdata[REAR_MODULE_FW_VERSION+10] == 'M'*/)
	{
		uint32_t PAF_err = 0;
		uint32_t PAF_bit = 0;
				...
				struct cam_eeprom_memory_block_t *block)
{
	...
	if (rc < 0) {
		CAM_ERR(CAM_EEPROM,"%s:(%d) write initial state failed\n", __func__, __LINE__);
		return rc;
	}
	rc = camera_io_dev_read(&e_ctrl->io_master_info, S5K5E9_OTP_BANK_SELECT_REGISTER, &OTP_Bank, addr_type,data_type);

	memptr[0] = OTP_Bank;
	
	CAM_INFO(CAM_EEPROM, "read OTP_Bank: %d\n", OTP_Bank);
#ifdef CONFIG_S5K5E9YX
	switch (OTP_Bank) {
		case 0:
		case 1:
		...
		case 0x1F:
			...
			break;
		default:
			CAM_INFO(CAM_EEPROM, "%s: Bank error : Bank(%d)\n", __func__, OTP_Bank);
			return -EINVAL;
	}
	CAM_INFO(CAM_EEPROM, "read page: 0x%x block->num_map=%u \n", page, block->num_map);
#endif
	...
	for (j = 0; j < block->num_map; j++) {
		...
		CAM_INFO(CAM_EEPROM, "emap[%d].mem.addr=0x%x block->mapdata=%p\n", j, emap[j].mem.addr, block->mapdata);
		if (emap[j].mem.valid_size) {
			CAM_INFO(CAM_EEPROM, "%s: %d\n" , __func__, __LINE__);
			...
			rc = camera_io_dev_write(&(e_ctrl->io_master_info), &i2c_reg_settings);
			if (rc < 0) {
				CAM_ERR(CAM_EEPROM,"%s:(%d) write page failed\n", __func__, __LINE__);
				return rc;
			}	
			...
			rc = camera_io_dev_write(&(e_ctrl->io_master_info), &i2c_reg_settings);
			if (rc < 0) {
				CAM_ERR(CAM_EEPROM,"%s:(%d) set read mode failed\n", __func__, __LINE__);
				return rc;
			}
		
			rc = camera_io_dev_poll(&e_ctrl->io_master_info, S5K5E9_OTP_ERROR_FLAG_REGISTER, 0x01 , 0 ,data_type, addr_type , 1 );
			if (rc < 0) {
				CAM_ERR(CAM_EEPROM,"%s:(%d) pool read byte failed\n", __func__, __LINE__);
				return rc;
			}
			...
			CAM_INFO(CAM_EEPROM, "%s: %d page %d start_addr = 0x%X\n",
			...
			while (total_bytes_to_read > 0) {
				CAM_INFO(CAM_EEPROM, "page_cnt [%d] read_bytes : %d total_bytes_to_read = %d\n",
					next_page_count, read_bytes, total_bytes_to_read);
					
				rc = camera_io_dev_read_seq(&e_ctrl->io_master_info, start_addr, memptr, addr_type,read_bytes);
				if (rc < 0) {
					CAM_ERR(CAM_EEPROM,"%s:(%d) read failed\n", __func__, __LINE__);
					return rc;
				}
				...
				if (total_bytes_to_read > 0) {
					...
					rc = camera_io_dev_write(&e_ctrl->io_master_info, &finish_read_otp);
					if (rc < 0) {
						CAM_ERR(CAM_EEPROM,"%s:(%d) finish_read_otp failed\n", __func__, __LINE__);
						return rc;
					}
					...
					i2c_reg_settings.size = 1;
					i2c_reg_array.reg_addr = 0x0A02;
					i2c_reg_array.reg_data = page + next_page_count;
					i2c_reg_settings.delay =0;
					i2c_reg_array.delay = 0;
					i2c_reg_settings.reg_setting = &i2c_reg_array;
...
#if defined(CAM_EEPROM_DBG)
	for (j = 0; j < block->num_data; j += 16) {
		CAM_INFO(CAM_EEPROM, "memptr[%03X]: %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X %02X\n",
		    j, memptr[j],memptr[j+1],memptr[j+2],memptr[j+3],memptr[j+4],memptr[j+5],memptr[j+6],memptr[j+7],
		    memptr[j+8],memptr[j+9],memptr[j+10],memptr[j+11],memptr[j+12],memptr[j+13],memptr[j+14],memptr[j+15]);
	}
	for (j = 0; j < block->num_data; j += 16) {
		CAM_INFO(CAM_EEPROM, "memptr[%03X]: %c %c %c %c %c %c %c %c %c %c %c %c %c %c %c %c\n",
		    j, memptr[j],memptr[j+1],memptr[j+2],memptr[j+3],memptr[j+4],memptr[j+5],memptr[j+6],memptr[j+7],
		    memptr[j+8],memptr[j+9],memptr[j+10],memptr[j+11],memptr[j+12],memptr[j+13],memptr[j+14],memptr[j+15]);
	}
#endif
	CAM_INFO(CAM_EEPROM, "%s: Read OTP success %d\n" , __func__, __LINE__);	
	return rc;
}
	...
	struct cam_eeprom_ctrl_t *e_ctrl)
{
	...
	if (e_ctrl->soc_info.index == 0)
		rc = cam_eeprom_dump(e_ctrl->soc_info.index,
			e_ctrl->cal_data.mapdata, 0, 0x1300);
	/*else if (e_ctrl->soc_info.index == 1)
		rc = cam_eeprom_dump(e_ctrl->soc_info.index,
			e_ctrl->cal_data.mapdata, 0, 0x1586);
	else if (e_ctrl->soc_info.index == 3)
		rc = cam_eeprom_dump(e_ctrl->soc_info.index,
			e_ctrl->cal_data.mapdata, 0, 0x1000);
	*/
#endif
	...
	case CAM_EEPROM_PACKET_OPCODE_INIT:
		...
		if (e_ctrl->userspace_probe == false) {
...

			{
				...
				if (e_ctrl->soc_info.index == CAM_EEPROM_IDX_DEPTH){			
