...
#if defined(CONFIG_CCIC_ALTERNATE_MODE)
	/* WA for Water detection in LPM mode
		If there is a oscillation in SBU line treated as water.
	*/
	if(lpcharge)
	{
		int gpio_dp_sw_oe;
		...
		gpio_dp_sw_oe = of_get_named_gpio(np, "usbpd,aux-en-gpio", 0);
