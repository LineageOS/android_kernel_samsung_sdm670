...
static inline char *secdp_dex_adapter_to_string(enum dex_adapter_t dongle)
{
	switch (dongle) {
	case DEX_ADAPTER_NONE:    return DP_ENUM_STR(DEX_ADAPTER_NONE);
	case DEX_ADAPTER_DEXSTN:  return DP_ENUM_STR(DEX_ADAPTER_DEXSTN);
	case DEX_ADAPTER_HG950:   return DP_ENUM_STR(DEX_ADAPTER_HG950);
	case DEX_ADAPTER_MPA2:    return DP_ENUM_STR(DEX_ADAPTER_MPA2);
	case DEX_ADAPTER_DEXPAD:  return DP_ENUM_STR(DEX_ADAPTER_DEXPAD);
	case DEX_ADAPTER_DEXCBL:  return DP_ENUM_STR(DEX_ADAPTER_DEXCBL);
...
#define SECDP_MAX_RESOLUTION_4K30		/* when max dp resolution is 4k@30hz, change to QHD@60hz */
...
#if defined (CONFIG_SEC_DISPLAYPORT) && defined(CONFIG_COMBO_REDRIVER)
