...
static int sx9330_set_offset_calibration(struct sx9330_p *data)
{
	...
	if (retries >= 50)
		pr_info("[SX9330]: %s - checking COMPSTAT Failed!\n", __func__);
...
static void sx9330_initialize_variable(struct sx9330_p *data)
{
	...
	data->skip_data = false;
	data->state = IDLE;
	/* SX9330_ADCFILTPH1_REG = SX9330_ADCFILTPH0_REG + (1 << (4 + MAIN_SENSOR) */
	/* Real threshold = data->touchTh * data->touchTh / 2 */
...
static void sx9330_change_threshold(struct sx9330_p *data, u32 threshold)
{
	...
	pr_info("[SX9330]: %s - reg : 0x%x before \n", __func__, reg);
	...
	pr_info("[SX9330]: %s - reg : 0x%x changed \n", __func__, reg);
}
