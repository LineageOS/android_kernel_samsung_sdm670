		 ...
		 size_t count)
{
	...
	if (data->usb_earjack) {
		if (earjack == 1) {
			cmd = CMD_EP_USB_ON;
			a96t3x6_i2c_write(data->client, REG_TSPTA, &cmd);
			SENSOR_INFO("USB-Earjack is inserted\n");
		} else {
			cmd = CMD_EP_USB_OFF;
			a96t3x6_i2c_write(data->client, REG_TSPTA, &cmd);
			SENSOR_INFO("USB-Earjack is removed\n");
		}
	} else {
				...
				struct device_attribute *attr, char *buf)
{
	...
	while (retry--) {
		...
		if ((checksum[0] != data->checksum_h_bin) ||
			...
			SENSOR_INFO("Checksum err CRC:%02x%02x, BIN:%02x%02x\n",
					checksum[0], checksum[1],
					data->checksum_h_bin,
					data->checksum_l_bin);
		} else {
			SENSOR_INFO("Checksum normal");
			break;
		}
	}
...
static int a96t3x6_parse_dt(struct a96t3x6_data *data, struct device *dev)
{
	...
	ret = of_property_read_u8(np, "a96t3x6,usb_earjack",
		&data->usb_earjack);
	if (ret < 0) {
		SENSOR_INFO("failed to get the earjack noise dt\n");
		data->usb_earjack = 0;
	} else {
		SENSOR_INFO("grip earjack noise block is adjusting %d \n", (int)data->usb_earjack);
		data->usb_earjack = 1;
	}
